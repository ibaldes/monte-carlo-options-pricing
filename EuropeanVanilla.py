...
#### Define a function to generate the terminal stock price, assuming risk-neutral probability for the discounted stock price, following a random walk.

def generate_terminal_price(S, r, sigma, t, T, n_steps=100, seed=0):

	'''
	Generates terminal stock price, using Monte-Carlo, given evolution under the risk-neutral measure for the discounted stock price 
...
def generate_terminal_price_ForGreeks(S, r, sigma, t, T, n_steps=100, seed=0):
	...
	return(terminal_price, terminal_price_smaller_S, terminal_price_larger_S, terminal_price_smaller_sigma, terminal_price_larger_sigma, terminal_price_smaller_t, terminal_price_larger_t, terminal_price_smaller_r, terminal_price_larger_r)
...
def MonteCarloVanillaEuropeanCall(S, K, r, sigma, t, T, n_simulations=250000, n_steps=100, BaseSeed=StandardBaseSeed):
	...
	with mp.Pool() as pool:
		terminal_price_array = pool.starmap(generate_terminal_price, args)
	
	#### 
	terminal_price_array = np.array(terminal_price_array)
...
def MonteCarloVanillaEuropeanCallWithGreeks(S, K, r, sigma, t, T, n_simulations=250000, n_steps=100, BaseSeed=StandardBaseSeed):
	...
	with mp.Pool() as pool:
		full_terminal_price_array = pool.starmap(generate_terminal_price_ForGreeks, args)

	#### 
	full_terminal_price_array = np.array(full_terminal_price_array)
	
	full_terminal_price_df = pd.DataFrame(full_terminal_price_array, columns=['terminal_price', 'terminal_price_smaller_S', 'terminal_price_larger_S', 'terminal_price_smaller_sigma', 'terminal_price_larger_sigma', 'terminal_price_smaller_t', 'terminal_price_larger_t', 'terminal_price_smaller_r', 'terminal_price_larger_r'])
